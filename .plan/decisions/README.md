...
`docs/decisions/` already holds thirteen accepted ADRs and is the permanent home;
...
| [003](ADR-CANDIDATE-003-metal-build.md) | Metal compute for Apple GPUs | `Promoted → ADR-013` | Passed (was v1) | v1's only GPU compute lane; same `Backend` trait. Original option A superseded |
...
| [011](ADR-CANDIDATE-011-daemon-transport.md) | Local daemon transport | `Promoted → ADR-012` | Passed (was `QM-0033`) | HTTP + Server-Sent Events |
...
| [018](ADR-CANDIDATE-018-tensor-id.md) | Canonical tensor ID generation | `Promoted → ADR-011` | Passed (was `QM-0020`) | Keep BLAKE3 over `(model_id, raw_name)` |
